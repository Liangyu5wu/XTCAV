...
%    'FACET' {'LI20'}; ...
    'FACET' {'IN10' 'LI20'}; ... % IN10 temporarily gone...
...
handles.sector.LI20.profMADList={ ...
    'USTHZ' 'USOTR' 'IPOTR1' 'IPOTR2' 'DSOTR' 'DTOTR2' 'WDSOTR' 'PRDMP'};
handles.sector.IN10.profMADList={ ...
    'PR10711' 'PR10571' 'PR10921' 'WS10561'}; % BHR 8/15/24 swapped 711 & 571
...
%disp(handles.sector.tcavDevList);
%disp(handles.sector.profDevList);
str={'Off' 'On'};
...
[pact,pdes]=control_phaseGet(sector.tcavMADList{1});
if strcmp(handles.measureTcavName,'TCAV:LI20:2400')
    handles.measurePhaseOrigVal = pdes; %FACET XTCAV PACT = nonsense when OFF (no PAD meas)
else
    handles.measurePhaseOrigVal = pact;
end
...
elseif strcmp(pv,'TCAV:DMPS:360')
    ...
    val=control_phaseGet(pv);
elseif strcmp(pv,'KLYS:LI10:51')
    val=control_phaseSet(pv,val,0);pause(7.); % for FACET IN10 TCAV
else
    disp(pv);
...
if val
    ...
    pDesSigned=mean([handles.measurePhaseRange{:}]);
    if ismember(handles.measureTcavKlys,handles.acclList)
        pDes=val*pDes;
    elseif val == -1 && strcmp(pv,'KLYS:LI10:51') % this is some mumbo jumbo because phase seems to be wrapping wrong way - sgess
        pDesSigned=util_phaseBranch(pDesSigned+180,0);
    elseif val == -1
    ...
    elseif strcmp(pv,'TCAV:DMPS:360')
        ...
        pAct=control_phaseGet(pv);
    elseif strcmp(pv,'KLYS:LI10:51')
        %pAct=control_phaseSet(pv,pDes,0);pause(7.); % for FACET IN10 TCAV
        pAct=control_phaseSet(pv,pDesSigned,0);pause(7.); % for FACET IN10 TCAV
    else
...
elseif strcmp(sector.tcavDevList{1},'TCAV:DMPS:360')
    ...
    pAct=control_phaseGet(sector.tcavDevList{1});
elseif strcmp(sector.tcavDevList{1},'KLYS:LI10:51')
    pAct=control_phaseSet(sector.tcavDevList{1},pDes,0);pause(7.); % for FACET IN10 TCAV
else
...
if strcmp(handles.measureTcavName,'TCAV:LI20:2400'), plane='x';end  % added XTCAVF
...
if strcmp(handles.measureTcavName,'TCAV:LI20:2400'), opts.plane='x';end  % added XTCAVF
if 1
%    opts.unitsT='ps';
%    scl=1e12/2856e6/360;
